...
<br/>
<form action="/admin/advertisement/edit/do" method="post" enctype="multipart/form-data">
    <center>
    <div>
        <div>
        ...
        <div>
            <input type="submit" class="btn btn-primary" value="发布广告"/>
        </div>
        <div>
...
<div>
    <c:forEach items="${photoList}" var="photo">
            <img src="/static/images/${photo.photoName }" width="380" height="250" >
            <input type="text" class="form-control" name="id" id="id" value="${photo.id }" hidden="hidden">
            <button type="button" class="btn btn-primary" onclick="useit('${photo.id }') ">使用</button>
            <button type="button" class="btn btn-danger" onclick="ifdelete('${photo.id }') ">删除</button>
    </c:forEach>
</div>
<div>
</div>
...
<script>
    ...
    function useit(id) {
        layer.confirm('确定use该广告吗?', {
            btn: ['确定','取消'] //按钮
        }, function(){
            $.ajax({
                type: 'POST',
                url: '/admin/advertisement/use',
                datatype:'json',
                data:{"id":id},
                success: function(data){
                    if(data['stateCode']==1){
